...
public:
	...
	void operator+(T item);	// Part 4: overload + for addAtEnd(T item)
	...
	T* getAtEnd();
	T* getAt(int p);
	T* operator/(int p);	// Part 4: overload / for getAt(int p)

	void setAtStart();
	void setAtEnd();

	int countList(T item);	// Part 2: add a new function
...
// Part 2: add a new function
template <typename T>
int LinkedList<T>::countList(T item)
{
	int count = 0;
	current = first;
	for (int i = 0; i < size; i++) {
		if (current->item == item) count++;
		current = current->next;
	}
	...
	//setAtStart();
	//for (int i = 0; i < size; i++) {
	//	T* ptr_current_item = getNext();
	//	if (*ptr_current_item == item) count++;
	//}

	return count;
}

// Part 4: overload + for addAtEnd(T item)
template <typename T>
...
// Part 4: overload / for getAt(int p)
template <typename T>
T* LinkedList<T>::operator/(int p)
{
	return getAt(p);
}
